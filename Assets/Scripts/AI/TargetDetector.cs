...
public class TargetDetector : Detector
{
    ...
    public override void Detect(AIData aiData)
    {
        ...
        if (playerCollider != null)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(transform.position, directionToPlayer, targetDetectionRange, obstaclesLayerMask);

            if (hit.collider != null && (playerLayerMask & (1 << hit.collider.gameObject.layer)) != 0)
            {
                Debug.DrawRay(transform.position, directionToPlayer * targetDetectionRange, Color.magenta);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        for(int i = 0; i < colliders.Count; i++)
            Gizmos.DrawSphere(colliders[i].position, 0.3f);
